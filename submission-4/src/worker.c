    ...
    while (millis() - start < 500) { 
        ...
        if (packetSize) {
            ...
            if (!deserializeJson(doc, incoming)) {
                if (doc["id"] == deviceID && doc["seq"] == mySeq && doc["type"] == "ACK") return true;
            }
        }
...
void mqttCallback(char *topic, byte *payload, unsigned int length) {
    ...
    if (strcmp(topic, mqtt_command_topic) == 0) {
        ...
        if (target && strcmp(target, deviceID) == 0) {
...
void SensorTask(void *pvParameters) {
    ...
    for (;;) {
        ...
        data.lightReading = floor( (4095 - rawLdr)) / 100 ; // Divide values by 100 rounded to 2 decimal places
...
void NetworkTask(void *pvParameters) {
    ...
    Serial.println("\nWiFi Connected");
    WiFi.localIP().toString().toCharArray(deviceID, WiFi.localIP().toString().length());
