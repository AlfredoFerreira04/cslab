...
static void log_alert(const char *message) {
    ...
    printf("[%s] %s\n", timebuf, message);
    if (alert_log) { fprintf(alert_log, "[%s] %s\n", timebuf, message); fflush(alert_log); }
}

void *mqtt_thread_func(void *arg) {
    while (!stop_server) { MQTTClient_yield(); usleep(100 * 1000); }
    return NULL;
}
...
void send_actuator_command(const char *type, const char *action) {
    ...
    if (!stop_server && difftime(now, last_global_command_time) < COMMAND_COOLDOWN) return;
...
static device_t *find_device_by_id(const char *id) {
    for (int i = 0; i < device_count; ++i) if (strcmp(devices[i].id, id) == 0) return &devices[i];
    return NULL;
}

static device_t *add_or_get_device(const char *id, struct sockaddr_in *addr) {
    device_t *d = find_device_by_id(id);
    if (d) { d->addr = *addr; d->last_seen = time(NULL); return d; }
    if (device_count >= MAX_DEVICES) return NULL;
    d = &devices[device_count++];
    strncpy(d->id, id, sizeof(d->id) - 1);
    d->id[sizeof(d->id) - 1] = '\0';
    d->temperature = 0; d->light = 0; d->dateObserved[0] = '\0';
    d->addr = *addr; d->has_seq = 0; d->last_seq = -1; d->last_seen = time(NULL);
    select_actuator();
    return d;
}
...
static void send_ack(int sockfd, struct sockaddr_in *client_addr, socklen_t addrlen, const char *id, long seq) {
    ...
    if (out) { sendto(sockfd, out, strlen(out), 0, (struct sockaddr *)client_addr, addrlen); free(out); }
    cJSON_Delete(ack);
}
...
int mqtt_message_arrived(void *context, char *topicName, int topicLen, MQTTClient_message *message) {
    ...
    if (strcmp(topicName, MQTT_PREF_TEMP_TOPIC) == 0) {
        ...
        if (sscanf(payload, "%lf,%lf", &min, &max) == 2) {
            pref_temp_min = min; pref_temp_max = max;
...
int main() {
    ...
    while (!stop_server) {
        ...
        if (n < 0) { if (errno == EINTR && stop_server) break; continue; }
    ...
    last_sent_heat_state = -1; last_sent_light_state = -1;
